  ...
  <div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px;text-align:center;">
        <p style="font-size:22px;color:#64748b;margin-bottom:6px;letter-spacing:0.05em;">累積期數</p>
        <p :style="{ fontSize:'1.8rem', fontWeight:'900', color: accent, fontFamily:'monospace' }">{{ totalDraws }}</p>
      </div>
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px;text-align:center;">
        <p style="font-size:22px;color:#64748b;margin-bottom:6px;letter-spacing:0.05em;">歷史均值</p>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:20px;padding:20px;">
      <h3 style="font-size:26px;font-weight:700;color:#f1f5f9;margin-bottom:14px;display:flex;align-items:center;gap:6px;">
        🔥 歷史上熱門號碼
      </h3>
      <div style="display:flex;gap:10px;justify-content:center;">
        <div v-for="(item, i) in hotNumbers" :key="item.num" style="display:flex;flex-direction:column;align-items:center;gap:6px;">
          ...
          <span style="font-size:20px;color:#64748b;">{{ item.count }}次</span>
        </div>
      </div>
    </div>
    ...
    <!-- Recent draws -->
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:20px;padding:20px;">
      <h3 style="font-size:26px;font-weight:700;color:#f1f5f9;margin-bottom:14px;">📅 近10期開獎紀錄</h3>
      <div style="display:flex;flex-direction:column;gap:10px;">
        <div v-for="draw in recentDraws" :key="draw.draw_id"
          ...
          <span style="font-size:22px;color:#475569;min-width:82px;">{{ draw.date }}</span>
          <div style="display:flex;gap:5px;flex-wrap:wrap;">
            <span v-for="n in draw.numbers" :key="n"
              style="width:26px;height:26px;border-radius:50%;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;font-family:monospace;color:#cbd5e1;">
              {{ n.toString().padStart(2,'0') }}
            </span>
            <span v-if="draw.special_number" style="width:26px;height:26px;border-radius:50%;background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.3);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;font-family:monospace;color:#fbbf24;">
