  ...
  <div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div :style="{
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        ...
        <span v-if="latestPrediction" style="font-size:22px;color:#64748b;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:100px;padding:3px 10px;">
      ...
      <div v-if="!latestPrediction" style="text-align:center;padding:40px 0;color:#475569;">
        ...
        <p style="font-size:28px;">尚無預測資料</p>
        <p style="font-size:24px;margin-top:4px;">請等待 AI 每日自動生成</p>
      </div>
      ...
      <div v-else>
        ...
        <div style="background:rgba(0,0,0,0.2);border-radius:12px;padding:14px;margin-bottom:16px;border:1px solid rgba(255,255,255,0.05);">
          <p style="font-size:22px;font-weight:600;color:#64748b;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">📊 AI 統計洞察</p>
          <p style="font-size:26px;color:#94a3b8;line-height:1.6;">{{ latestPrediction.prediction.reasoning }}</p>
        </div>

        <!-- Combinations -->
        <div style="display:flex;flex-direction:column;gap:10px;">
          <p style="font-size:22px;font-weight:600;color:#64748b;letter-spacing:0.08em;text-transform:uppercase;">💡 推薦投注組合</p>
          <div v-for="(nums, strategy) in latestPrediction.prediction.combinations" :key="strategy"
            ...
            <span :style="{ fontSize:'24px', fontWeight:'700', color: strategyStyle(strategy).color, minWidth:'80px' }">{{ strategy }}</span>
            <div style="display:flex;gap:6px;flex-wrap:wrap;">
              <span v-for="n in nums" :key="n"
                :style="{
                  ...
                  fontSize:'24px', fontWeight:'700', fontFamily:'monospace', color:'#f1f5f9'
                }">
        ...
        <!-- Risk warning -->
        <p v-if="latestPrediction.prediction.risk_warning" style="font-size:22px;color:#475569;margin-top:12px;font-style:italic;">
    ...
    <div v-if="latestDraw" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:20px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
        ...
        <span style="font-size:22px;color:#64748b;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:100px;padding:3px 10px;">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <span v-for="n in latestDraw.numbers" :key="n"
          :style="{
            ...
            display:'flex', alignItems:'center', justifyContent:'center',
            fontSize:'26px', fontWeight:'800', fontFamily:'monospace', color: accent
          }">
          {{ n.toString().padStart(2,'0') }}
        </span>
        <div v-if="latestDraw.special_number" style="display:flex;align-items:center;gap:6px;">
          <span style="color:#475569;font-size:24px;">特別號</span>
          <span style="width:40px;height:40px;border-radius:50%;background:rgba(251,191,36,0.12);border:2px solid rgba(251,191,36,0.5);display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:800;font-family:monospace;color:#fbbf24;">
