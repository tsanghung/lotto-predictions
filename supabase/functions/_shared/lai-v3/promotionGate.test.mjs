...
test("family representatives keep one deterministic strongest non-uniform candidate", () => {
  const candidates = [
    ...
    { name: "bayes-b", family: "bayesian", evidence: passingEvidence({ brierCi: { lower95: 0.002, upper95: 0.02 } }) },
    { name: "bayes-a", family: "bayesian", evidence: passingEvidence({ brierCi: { lower95: 0.002, upper95: 0.02 } }) },
    { name: "transition-a", family: "transition", evidence: passingEvidence({ recent500Skill: 0.02 }) },
  ];
...
test("canary rejects approved weights that remove the permanent uniform baseline", () => {
  assert.throws(() => buildProductionWeights({
    baselineName: "uniform-null",
    currentChampion: { name: "bayes-v1", family: "bayesian", stage: "champion" },
    approvedWeights: { "bayes-v1": 1 },
    challenger: { name: "transition-v1", family: "transition", stage: "canary" },
...
test("champion weights reserve 0.25 for uniform and share 0.75 by excess loss", () => {
  const candidates = [
    { name: "bayes-v1", family: "bayesian", stage: "champion", evidence: passingEvidence({ meanExcessLoss: 0 }) },
    { name: "bayes-v2", family: "bayesian", stage: "champion", evidence: passingEvidence({ meanExcessLoss: -0.02, brierCi: { lower95: 0.002, upper95: 0.03 } }) },
    { name: "transition-v1", family: "transition", stage: "champion", evidence: passingEvidence({ meanExcessLoss: 0.1 }) },
    { name: "sequence-v1", family: "sequence", stage: "champion", evidence: passingEvidence({ recent500Skill: 0 }) },
  ];
...
test("no eligible challenger retains a valid prior champion or falls back to uniform-null", () => {
  const prior = { name: "prior-v1", family: "prior", stage: "champion", evidence: passingEvidence() };
  assert.deepEqual(buildProductionWeights({
    baselineName: "uniform-null",
    currentChampion: prior,
    candidates: [{ name: "bad-v1", family: "bad", stage: "champion", evidence: passingEvidence({ recent100Skill: 0 }) }],
  }), {
    "uniform-null": 0.25,
    "prior-v1": 0.75,
    "bad-v1": 0,
  ...
  assert.deepEqual(buildProductionWeights({
    baselineName: "uniform-null",
    candidates: [{ name: "bad-v1", family: "bad", stage: "champion", evidence: passingEvidence({ recent500Skill: -0.01 }) }],
  }), {
    "uniform-null": 1,
    "bad-v1": 0,
  });
...
test("family isolation rejects a canary from the active champion family", () => {
  assert.throws(() => buildProductionWeights({
    ...
    currentChampion: { name: "bayes-v1", family: "bayesian", stage: "champion" },
    approvedWeights: { "uniform-null": 0.25, "bayes-v1": 0.75 },
    challenger: { name: "bayes-v2", family: "bayesian", stage: "canary" },
